...
import { Constant, CONSTANTS } from "./constant.ts";
import { Token, TokenType } from "./tokens.ts";
import { Operator } from "./operator.ts";
import { Func } from "./func.ts";
import { Operation, OPERATIONS, Parenthesis } from "./operation.ts";
...
class InvalidTokenError extends Error {
  constructor(token: Token) {
    super(
      `${token.start}|${token.end} - "${token.value}" is not a valid token.`,
    );
  }
}
...
class UnkownIdentifierError extends Error {
  constructor(token: Token) {
    super(
      `${token.start}|${token.end} - "${token.value}" is not a known identifier.`,
    );
  }
}
...
class UnkownOperatorError extends Error {
  constructor(token: Token) {
    super(
      `${token.start}|${token.end} - "${token.value}" is not a known operator.`,
    );
  }
}
...
export class Parser {
    ...
    while (this.output.length !== this.src.length) {
      ...
      switch (token.type) {
        ...
        case TokenType.OPERATOR: {
          // Negative number
          if (
            token.value === "-" && this.nextToken.type === TokenType.NUMBER &&
  ...
  private parseOperator(op: Operator): void {
    while (
      ...
      this.operation.length !== 0 && this.lastOperation instanceof Operator && (
        ...
        !(this.lastOperation instanceof Parenthesis) ||
        !this.lastOperation.isLeft
      )
    ) {
  ...
  private parseRightParenthesis(_rp: Parenthesis): void {
    while (
